...
public class Descente extends Observable implements Runnable  {
	 ...
	 * Recherche la prochaine directprochaine direction a suivre lorsque le point courant P
	 * est dans l'interieur strict du domaine.ion a suivre lorsque le point courant P
	...
	private void directionASuivreSiInterieur() {
		if (pb.gradientf(this.P).norme() < seuil) {
			this.finie = true;
		}
		else {
			this.direction = this.pb.gradientf(this.P).mult(-1);
		}
	...
	private void directionASuivreSiBord(Contrainte c) {
		if (pb.gradientf(this.P).norme() < seuil || c.getGradient().mult(-1)==pb.gradientf(this.P).mult(1/pb.gradientf(this.P).norme())) {
			this.finie = true;
		}
		else {
			if 	(c.getGradient().produitScalaire(pb.gradientf(this.P) )<0) {
				if (c.getVecteurUnitaireBord().produitScalaire(pb.gradientf(this.P))>0) {
					this.direction = c.getVecteurUnitaireBord().mult(-1);
				}
				else {
					this.direction = c.getVecteurUnitaireBord();
				}
			}
			else{
				this.direction = this.pb.gradientf(this.P).mult(-1);
	...
	private void directionASuivreSiCoin(Contrainte[] coin) {
		boolean kkt = true;
		//Contrainte[] coinutil=null;
		for (Contrainte i:coin) {
			if (i.getGradient().produitScalaire(pb.gradientf(this.P))>0) {
				kkt = false;
				//coin.remove(i);
			}
		}
		if (pb.gradientf(this.P).norme() < seuil || kkt == true ){
			this.finie = true;
		}
		else if ((coin[0].getGradient().produitScalaire(pb.gradientf(this.P))>0) &&(coin[1].getGradient().produitScalaire(pb.gradientf(this.P))>0)) {
			this.direction = this.pb.gradientf(this.P).mult(-1);	
		}
		else if ((coin[0].getGradient().produitScalaire(pb.gradientf(this.P))>0) &&(coin[1].getGradient().produitScalaire(pb.gradientf(this.P))<0)) {
			if (coin[1].getVecteurUnitaireBord().produitScalaire(pb.gradientf(this.P))>0) {
				this.direction = coin[1].getVecteurUnitaireBord().mult(-1);
			}
			else {
				this.direction = coin[1].getVecteurUnitaireBord();
			}
		}
		else if ((coin[0].getGradient().produitScalaire(pb.gradientf(this.P))<0) &&(coin[1].getGradient().produitScalaire(pb.gradientf(this.P))>0)) {
			if (coin[0].getVecteurUnitaireBord().produitScalaire(pb.gradientf(this.P))>0) {
				this.direction = coin[0].getVecteurUnitaireBord().mult(-1);
			}
			else {
				this.direction = coin[0].getVecteurUnitaireBord();
			}
		}
