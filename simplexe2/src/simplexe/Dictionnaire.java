...
public class Dictionnaire extends Observable {
	...
	public int chercherIndiceVariableSortante(int jE)  {
		...
		int ind = 0;
		for (int i =1;i<=nbVarBase;i++) {
			if ((D[i][0]/D[i][jE])>1/epsilon) {return 0;}
			if ((-D[i][0]/D[i][jE])>1/epsilon) {return 0;}
			if ((temp<(-D[i][0]/D[i][jE]))) {
				temp = -D[i][0]/D[i][jE];
				ind = i;
			}
		}
	...
	public void pivoter(int iS, int jE) {
		D[iS][0] =- D[iS][0]/D[iS][jE];
		for(int j = 1;j<=nbVarHorsBase;j++ ) {
			if (j!=jE) {
				D[iS][j] = -D[iS][j]/D[iS][jE];
			}
		}
		D[iS][jE] = 1/D[iS][jE];
		for (int i = 0; i<=nbVarBase;i++) {
			if (i != iS) {
				for (int j = 0; j<=nbVarHorsBase;j++) {
					if(j!=jE){
						D[i][j] = D[i][j]+D[iS][j]*D[i][jE];
					}
				}
				D[i][jE] = D[i][jE]*D[iS][jE];
			}
		}
	 ...
	 * <br>Dans lDans les commentaires du programme, on des autres cas, effectue une etape de la methode du simplexe en choisissant une 
	...
	public void uneEtape(int jE) {//on obtient un dictionnaire dégénéré
		int iS;
		if (D[0][jE] <= 0) {			
